...
class KMeans:
    ...
    def assign_centroids(x, centroids):
        ...
        distances = np.array(
            [
                [KMeans.euclidean_distance(row, centroid) for centroid in centroids]
    ...
    def recalculate_centroids(x, centroids):
        for i in range(len(centroids)):
            ...
            if relevent_points.shape[0] == 0:
                centroids[i][:] = (
                    x.drop("Cluster", axis=1).iloc[np.random.choice(x.shape[0])].values
                )
            else:
    ...
    def train(self, x, max_iters=10000, verbose=False):
        ...
        for _ in range(self.retry):
            ...
            self.centroids = (
                ...
                .iloc[np.random.choice(x.shape[0], self.n_clusters, replace=False)]
                .values
            )
